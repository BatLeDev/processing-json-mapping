...
module.exports = async (processingConfig, tmpFile, axios, log) => {
  ...
  const datasetSchema = await processingConfig.columns.reduce((acc, column) => {
    const schema = { key: column.columnPath.replace('.', '') }
    if (column.multivalued) {
      schema.separator = ';'
    }

    if (column.columnName) {
      const dotIndex = column.columnPath.indexOf('.')
      if (dotIndex !== -1) {
        schema['x-originalName'] = column.columnPath
      }
      schema.title = column.columnName
    }
  ...
  await log.info(JSON.stringify(datasetSchema))
